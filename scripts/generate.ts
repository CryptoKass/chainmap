...
import {capitalizeWords, getWorkingRPCs, loadNetworkMap, writeFile} from './utils.js'
...
const formatNetworkMap = (networks: NetworkMap) => {
	for (let [networkName, networkMap] of Object.entries(networks)) {
		for (let [chainName, chainInfo] of Object.entries(networkMap)) {
			...
			if (chainInfo.iconUrl != undefined) chainInfo.iconUrl = `${ASSET_URL}/${chainInfo.iconUrl}`
		}
	}
}
...
const generateIndexCode = (networks: NetworkMap) => {
	...
	indexSrc += `import { asProxyWithChainId } from "./utils.js";\n`
	indexSrc += `import chainIds from "./chainIds.js";\n`
